	      ...
	      for (int jc = 1; jc <= nlocal[Y]; jc++) {
            ...
            //printf("counter: %d \n", counter);
				...
				if (fixed_potential[counter] == 1 || fixed_potential[counter] == -1 || fixed_potential[counter] == 2){
				  ...
				  dpsi
					= psidata[addr_rank0(nsites, index + xs)] //+ psi_ext[ic+1]
					+ psidata[addr_rank0(nsites, index - xs)] //+ psi_ext[ic-1]
					+ psidata[addr_rank0(nsites, index + ys)]
					+ psidata[addr_rank0(nsites, index - ys)]
					+ psidata[addr_rank0(nsites, index + zs)]
					+ psidata[addr_rank0(nsites, index - zs)]
					- 6.0*psidata[addr_rank0(nsites, index)]; //- 6.0*psi_ext[ic];
...
int PSI_ext(psi_t * psi) {
  ...
  //psi_ext = calloc(psi->nsites, sizeof(int));
  //psi_ext = calloc(1024, sizeof(int));
  const double pi = 3.14159265358979323846;
  double E0 = 0.0001;
  double K = 2 * pi * 16 / ntotal[X];
